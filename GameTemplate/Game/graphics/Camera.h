...
public:
	...
	//ビュー行列の逆行列を取得
	CMatrix GetViewMatrixInv()
	{
		return m_viewMatrixInv;
	}
	
	//カメラの回転行列を取得。
	CMatrix& GetCameraRotation()
	{
...
private:
	...
	CMatrix m_viewMatrixInv = CMatrix::Identity();	//ビュー行列の逆行列
	CMatrix	m_cameraRotation = CMatrix::Identity();	//カメラの回転行列。
